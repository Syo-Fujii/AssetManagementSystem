...
-- 備品一覧参照ビュー
CREATE 
OR REPLACE view equip_list_view AS WITH equip_work(lending_name, lending_count) AS ( 
    SELECT
...
, stock_work(stock_name, stock_count) AS ( 
    SELECT
...
, unknown_work(unknown_name, unknown_count) AS ( 
    SELECT
    ...
    WHERE
        stock_master.del = 0 
        AND DATEDIFF(CURRENT_DATE (), stock_data.confirmed_date) > 365 
    GROUP BY
        stock_type_master.name
        , stock_master.stock_type
) 
...
OR REPLACE view equip_detail_view AS SELECT DISTINCT
    ...
    , case 
        when DATEDIFF(CURRENT_DATE (), stock_data.confirmed_date) > 365 
            then '不明' 
...
WHERE
    stock_master.del = 0 and stock_data.del=0 and stock_data.return_date is null
ORDER BY
